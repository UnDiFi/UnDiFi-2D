! setbndrynodeptr itself moved to mod_setbndrynodeptr.f90 (Phase 2/3
! interface fix) -- its allocatable intent(out) dummies need an
! explicit interface, which only a module procedure gives for free.
! myroutine/findcolours/setbndrynodelist/nearby/check below have no
! allocatable dummies, so calling them via implicit `external`
! interfaces (as setbndrynodeptr's own body, and readmesh.f90's `call
! check`, still do) remains standard-conforming.
!
