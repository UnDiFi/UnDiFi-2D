...
! source_utils/Triangle2grd/r_files_mod.f90, unchanged except the
! hardcoded units 10-13 -> newunit and one extra log_unit argument for
! the single diagnostic write that used to go to stdout (redirected via
! shell into log/triangle2grd.log for the standalone executable; now
! written to that same file directly since there's no shell redirect
! once this runs in-process). See na2vvvv.f90 for the port rationale.
...
! =====================================
  ...
  contains
    ...
    subroutine r_files(xy_node,att_node,ele,neigh,b_edge,bmk_f,nb_bed,fname,lenfname,type,bmk_edge,log_unit)
...
!aldo
      ...
      ! ======================
      open(newunit=u10, file=fname(1)(1:lenfname(1)), status='old')
      read(u10,*) nb_node, temp, nb_att
      do i=1,nb_node
         read(u10,*) temp, xy_node(1,i), xy_node(2,i), (att_node(j,i),j=1,nb_att)
      end do
      close(u10)
      ...
      ! ==========================
      open(newunit=u11, file=fname(2)(1:lenfname(2)), status='old')
      read(u11,*) nb_ele
      do i=1, nb_ele
         read(u11,*) temp, ele(1,i), ele(2,i), ele(3,i)
      end do
      close(u11)
      ...
      ! ============================
      open(newunit=u12, file=fname(3)(1:lenfname(3)), status='old')
      read(u12,*)
      do i=1,nb_ele
         read(u12,*) temp, neigh(1,i), neigh(2,i), neigh(3,i)
      end do
      close(u12)
...
! **********************
      ...
      open(newunit=u13, file=fname(4)(1:lenfname(4)), status='old')
      read(u13,*) nb_edge
      j = 0
      do i=1, nb_edge
         read(u13,*) temp, s1, s2, mark
         if ((mark/=0)) then
            j = j + 1
            b_edge(1,j) = s1
            b_edge(2,j) = s2
      ...
      do i = 1, 10
         j = howmany(i)
         if(j.NE.0)write(log_unit,*)' subroutine r_files: faces coloured ',i,' are ',j
      end do
