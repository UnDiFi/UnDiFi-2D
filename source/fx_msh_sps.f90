...
!     .. local scalars ..
  integer(i4) i, ibfac, ish, isppnts, ilist,&
...
! Phase 3.2 increment 6: each isppnts' remesh_boundary now dispatches
! through the special_point_t registry instead of a 16-way if/elseif.
! ibfac is passed as the ibfac_carry argument, threading the running
! boundary-edge count through every call exactly as it did through the
! legacy if/elseif's own shared local (see mod_special_point.f90's
! sp_remesh_if header). make_special_point error-stops on any code
! outside the 16 handled here, preserving the original's final
! 'condition not implemented!' stop.
  do isppnts = 1, nspecpoints
    call make_special_point(typespecpoints(isppnts), sp)
    call sp_unpack(sp, shinspps(:, :, isppnts))
    call sp%remesh_boundary(xysh(:, :, 1:nshmax), xyshu(:, :, 1:nshmax),&
    &xyshd(:, :, 1:nshmax), ibndfac, nodcod, xy, ishplistu, ishplistd,&
    &nshockpoints(1:nshmax), nbfac, ibfac)
  end do
