...
#                             nbegin, nsteps, eulfs, steady, testcase,            logfile
   ../../build/gfortran-debug/bin/UnDiFi-2D 0       501     false  true    "RegularReflection-2" | tee run.log

elif [ "$Solver" = "neo" ] && [ "$Mode" = "fitting" ] && [ "$Flow" = "unsteady" ]; then
#  modify NEO's input file for SF simulation
   cp NEO_data/textinput/inputfile-exp.txt.SF NEO_data/textinput/inputfile-exp.txt

   ../../build/gfortran-debug/bin/UnDiFi-2D 0       501     false  false   "RegularReflection-2" | tee run.log

elif [ "$Solver" = "eulfs" ] && [ "$Mode" = "fitting" ] && [ "$Flow" = "steady" ]; then
   ../../build/gfortran-debug/bin/UnDiFi-2D 0       501     true   true    "RegularReflection-2" | tee run.log

elif [ "$Solver" = "eulfs" ] && [ "$Mode" = "fitting" ] && [ "$Flow" = "unsteady" ]; then
   ../../build/gfortran-debug/bin/UnDiFi-2D 0       501     true   false   "RegularReflection-2" | tee run.log
