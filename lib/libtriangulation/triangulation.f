...
c
     ...
     &       ca_len .eq. 0.0D+00 ) then
          ...
          if ( ca_len .eq. 0.0D+00 .or. ab_len .eq. 0.0D+00 ) then
            ...
            a_angle = 
     &        arc_cosine ( ( ca_len**2 + ab_len**2 - bc_len**2 ) 
     &        / ( 2.0D+00 * ca_len * ab_len ) )
          ...
          if ( ab_len .eq. 0.0D+00 .or. bc_len .eq. 0.0D+00 ) then
            ...
            b_angle = 
     &        arc_cosine ( ( ab_len**2 + bc_len**2 - ca_len**2 ) 
     &        / ( 2.0D+00 * ab_len * bc_len ) )
          ...
          if ( bc_len .eq. 0.0D+00 .or. ca_len .eq. 0.0D+00 ) then
            ...
            c_angle = 
     &        arc_cosine ( ( bc_len**2 + ca_len**2 - ab_len**2 ) 
...
10    continue
     ...
     &          'TRIANGULATION_ORDER3_EDGE_CHECK - Warning!'
          write ( *, '(a)' ) '  An edge occurs twice,POS',I, col(3,i)
              return
            end if
...
c
      ...
      subroutine triangulation_order3_search ( node_num, node_xy, 
     &  triangle_order, triangle_num, triangle_node, triangle_neighbor, 
     &  p, seed, triangle_index, edge )

c*********************************************************************72
c
cc TRIANGULATION_ORDER3_SEARCH searches a triangulation for a point.
c
c  Discussion:
c
...
c
c    Output, integer EDGE, indicates the position of the point P
c    in triangle TRIANGLE_INDEX:
...
!         write ( *, '(a)' ) ' '
!         write ( *, '(a)' ) 
!    &      'TRIANGULATION_ORDER3_SEARCH - Fatal error!'
...
20    continue
      ...
      subroutine triangulation_order3_search_naive ( node_num, 
     &  node_xy, triangle_order, triangle_num, triangle_node, p, 
     &  triangle_index )

c*********************************************************************72
c
cc TRIANGULATION_ORDER3_SEARCH_NAIVE naively searches a triangulation.
c
c  Discussion:
c
