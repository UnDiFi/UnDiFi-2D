...
c-----------------------------------------------------------------------
 ...
 10     continue
	...
	integer nzmaxc, nzminc, nzmaxr, nzminr, nzcol, nzrow
        logical sym
c----------------------------------------------------------------------
...
c
 ...
 10 	continue
        ...
        subroutine diag_domi(n,sym,valued,a, ja,ia,ao,iao, 
     *                           ddomc, ddomr)
        implicit real*8 (a-h, o-z)
...
c number of diagonally dominant rows
 ...
 30	             	continue 
          		...
          		if (sym) then
             		  if (dsumr+dsumc .le. 3.0d0*aii) 
     *                       nddomc = nddomc + 1
             		else
             		  if (dsumc .le. 2.0d0*aii) nddomc = nddomc +1
             		  if (dsumr .le. 2.0d0*aii) nddomr = nddomr+1
          		endif
          	endif
 10     continue
 	if (sym) nddomr = nddomc

        ddomr = real(nddomr)/ real(n)
        ddomc = real(nddomc)/ real(n)
