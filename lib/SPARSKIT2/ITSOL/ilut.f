...
c----------------------------------------------------------------------c
c This Version Dated: August 10, 1995. Warning: meaning of some        c
c ============ arguments have changed w.r.t. earlier versions. Some    c
c              Calling sequences have also changed: ilut, ilutp.       c
c Contents:                                                            c
c-------------------------preconditioners------------------------------c 
c                                                                      c
...
c----------------------------------------------------------------------*
c     Author: Yousef Saad *May, 5, 1990, revised April, 1995           *
c----------------------------------------------------------------------*
c PARAMETERS                                                           
c-----------                                                           
c
...
c        and the largest lfil elements in the i-th row of U.           *
c                                                                      *
...
c
         do 170  j = j1, j2
            k = ja(j)
            t = a(j)
c##          if (abs(t) .lt. tol*tnorm .and. k .ne. ii) goto 170
            if (k .lt. ii) then
               lenl = lenl+1
               jw(lenl) = k
               w(lenl) = t
...
c     
c         if (abs(w(jj)) .le. tol*tnorm) goto 150
c     
c     get the multiplier for row to be eliminated (jrow).
c     
         fact = w(jj)*alu(jrow)
         if (abs(fact) .le. tol) goto 150
c     
...
c     
c      if (abs(s) .lt. tol*tnorm .and. jpos .eq. 0) goto 203
            if (j .ge. ii) then
c     
...
c     
         ...
         len = min0(lenl,lfil)
c     
c     weigh the elements before sorting 
c     
         do k=1, lenl 
            w(k) = w(k)*(tol+w(n+jw(k)))  
         enddo
c     sort 
...
c            if (abs(w(ii+k)) .gt. tol*abs(w(ii))) then 
c            if (w(ii+k)*w(ii+k) .gt. tol*abs(w(ii))) then 
            if (abs(w(ii+k)) .gt. tol*abs(w(ii))) then 
               len = len+1
               w(ii+len) = w(ii+k) 
               jw(ii+len) = jw(ii+k) 
            endif
...
c     divide by len+1 instead of len to avoid division by zero.
c     
         w(n+ii) = t / real(len+1)
c
...
c-----------------------------------------------------------------------
     ...
     *     iperm(2*n),ierr
      real*8 a(*), alu(*), w(2*n), droptol
c----------------------------------------------------------------------*
c       *** ILUTP preconditioner -- ILUT with pivoting  ***            *
c      incomplete LU factorization with dual truncation mechanism      *
c----------------------------------------------------------------------*
c author Yousef Saad *Sep 8, 1993 -- updated:                          * 
c April 6, 1995 -- Aug 1995.                                           *
c----------------------------------------------------------------------*
c on entry:
c==========
...
c
         do 170  j = j1, j2
            k = iperm(n+ja(j))
            t = a(j)
c            if (abs(t) .lt. droptol*tnorm .and. k .ne. ii) goto 170
            if (k .lt. ii) then
               lenl = lenl+1
               jw(lenl) = k
               w(lenl) = t
...
c     if fill-in element is small then disregard:
c     
c            if (abs(s) .lt. droptol*tnorm .and. jpos .eq. 0) goto 203
            if (j .ge. ii) then
c
c     dealing with upper part.
c
...
c     
 ...
 302	continue
c
c     save norm in w (backwards). Norm is in fact average abs value
c
        w(n+ii) = t / real(len+1)
c
c     store inverse of diagonal element of u
...
c----------------------------------------------------------------------* 
c     locals
      integer ju0,k,j1,j2,j,ii,i,lenl,lenu,lenl0,lenu0,jj,jrow,jpos,n2,
     *     jlev, min 
      real*8 t, s, fact 
      if (lfil .lt. 0) goto 998
c-----------------------------------------------------------------------
...
c     
         jlu(ii+1) = ju0
c     
c     reset double-pointer to zero (U-part) + levs 
c     
         do 308 k=1, lenu
            jw(n+jw(ii+k-1)) = 0
 308     continue
