...
c subroutines as afun, bfun, ..., gfun, hfun.
...
c           dn
c Where alpha is constant at each side of the boundary
c surfaces. Alpha, is represented by parameter al. Al,
c is expected to arrays that contains sufficient number of
c elements to specify the boundaries for the problem, 1-D needs two of
c each, 2-D needs 4 and 3-D needs 6, the order of the boundaries in
c the array is left(west), right(east), bottom(south), top(north), front,
c rear. Beta and gamma are functions of type real with three arguments
c that assumed to be x, y, z coordinates. These two functions are known
c subroutine 'addbc' as betfun and gamfun. They should following the
c same notion as afun ... hfun. For more restriction on afun ... hfun,
c please read the documentation follows the subroutine 'getsten', and,
c for more on betfun and gamfun, please refer to the documentation under
c subroutine 'fdaddbc'.
c
c The nodes are ordered using natural ordering, first x direction, then y,
c then z mesh size h is uniform and determined by grid points
...
c when calling relavent functions that have three parameters. Similarly,
c for 2-D case, z is assumed to be zero.
c
...
c and when ny is 1 nz must be 1.
c--------------------------------------------------------------------
c
...
c     condition, since in which case the value of the boundary point is
c     known.
c     Then in the second half, we will try to eliminate the boundary
c     points with known value (with Dirichlet boundary condition).
c
      kx = 1
      ky = nx
      kz = nx*ny
...
c
c     go through all the actual nodes with unknown values, collect all of
c     them to form a new matrix in compressed sparse row format.
c
 100  kx = 1
      ky = ux - lx + 1
      kz = (uy - ly + 1) * ky
...
c     alpha -- + beta u = gamma
c           dn
c The alpha is stored in array al. The six side of the boundary appares
...
c Beta and gamma appears as the functions, they are betfun and gamfun.
c The requirements for these two functions are:
c the return type is real*8(double presicion);
c they each acccpts three variables assumed to be x, y, z coordinates,
c and x, y, z should be in double precision (real*8) also;
c
c Exceptional situations are about handling of the corner pointes,
c first, when one side is Dirichlet boundary condition, betfun and
c gamfun have to take the values specified for that side;
c if both sides are Dirichlet, then, it doesn't matter, what's value
c beta and gamma, but it's suggested value of the side comes first in
c the above ordering is taken;
c When both sides are non-Dirichlet, then, an average has to be taken
c in setting the value of betfun and gamfun, in order to gaurantee a
c correct outcome.
c-----------------------------------------------------------------------
c     some constants
c
...
c     the actual dimension is lower then it should at least have one
c     dimension, the second dimension if it should have any must be y.
c     If only two dimension is used nz must be 1, if only one dimension
c     is used ny and nz both must be 1 (one).
c
