...
C
      ...
      IF(.NOT.LFLAG)THEN
         WRITE(6,FMT=*)
         WRITE(6,FMT=*) 'Cannot find file016.dat; therefore I CANNOT com
     &pute the skin friction'
         WRITE(6,FMT=*)
         GOTO 77
      ENDIF
C
...
C
C
         CALL triangulation_order3_boundary_edge_count_euler ( NPOIN,
     &   NELEM, NHOLE, NITEMS)
         WRITE(6,*)'The mesh has ',NBFAC,' boundary edges; triangulation
     &_order3_boundary_edge_count_euler has found ',NITEMS
         IF((NITEMS-NBFAC).NE.0)CALL EXIT(NITEMS-NBFAC)
         CALL triangulation_order3_boundary_edge_count( NELEM,
     &   ISTAK(LCELNOD), NITEMS)
         WRITE(6,*)'The mesh has ',NBFAC,' boundary edges; triangulation
     &_order3_boundary_edge_count has found ',NITEMS
         IF((NITEMS-NBFAC).NE.0)CALL EXIT(NITEMS-NBFAC)
C
     ...
     &                                            NITEMS, IFAIL )
            WRITE(6,*)'triangulation_order3_edge_check has returned ',
     &                IFAIL
            WRITE(6,*)'triangulation_order3_edge_check has found ',
     &                NITEMS,' boundary edges; mesh has ',NBFAC
            CALL EXIT(IFAIL)
         ENDIF
...
!
         ...
         write(6,*)'The Delaunay check has returned min angle ',
     &angle_min
         write(6,*)'in triangle ',angle_min_triangle
         write(6,*)'The Delaunay check has returned max angle ',
     &angle_max
