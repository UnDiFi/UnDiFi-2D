...
{
   echo ""
   echo -e "${bold} Usage: $0 ${RED}-s Solver${NOCOLOR} ${BLUE}-m Mode${NOCOLOR} ${GREEN}-f Flow${NOCOLOR} ${YELLOW}-i Iters${NOCOLOR}"
   ...
   echo -e "${CYAN}\t-i Iters: number of iterations ${NOCOLOR}"
   echo ""
   echo ""
...
# Print helpFunction in case parameters are empty
 if [ -z "$Solver" ] || [ -z "$Mode" ] || [ -z "$Flow" ] || [ -z "$Iters" ]
then
